...
function MenteeSignupPage() {
  ...
  const [highestDegreeCompletionYear, setHighestDegreeCompletionYear] =
    useState("");
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(
        `${import.meta.env.VITE_BASE_URL}/mentee/register`,
        newMentee,
      );
      ...
      if (error.response) {
        ...
        if (data.errors && Array.isArray(data.errors)) {
          ...
          data.errors.forEach((err) => {
            console.error(
              `Field: ${err.path || "Unknown"}, Message: ${err.msg}`,
            );
  ...
  return (
    <div className="w-full min-h-screen flex justify-center items-center bg-gray-900 py-10 px-4">
      <div className="bg-gray-800 p-6 md:p-8 lg:p-10 rounded-2xl shadow-xl w-full max-w-4xl">
        <h1 className="text-2xl md:text-3xl lg:text-4xl text-blue-400 font-bold text-center mb-6">
          Mentee Sign Up
        </h1>
        ...
        <form className="flex flex-col gap-6" onSubmit={handleSubmit}>
          <div>
            <h3 className="text-base md:text-lg lg:text-xl font-semibold text-gray-300 mb-4">
            ...
            {otherDegrees.map((degree, index) => (
              <div key={index} className="flex flex-col lg:flex-row gap-4 mb-4">
                <input
                  ...
                  onChange={(e) =>
                    handleOtherDegreeChange(index, "institute", e.target.value)
                  }
                  required
                />
                <input
                  ...
                  onChange={(e) =>
                    handleOtherDegreeChange(index, "degreeName", e.target.value)
                  }
                  required
                />
                <input
